to_late(Pos, Time, Customer_info):-
	get_customer_info(Customer_info,_, Pos2, _, Off, _),
	shortest_path(Pos, Pos2, Dist, _),
	Time+Dist > Off.

someone_else_is_better(Pos, Time, Customer_info):-
	get_customer_info(Customer_info, _, Pos2, On, _, Pickup_time),
	shortest_path(Pos, Pos2, Dist, _),
	abs(On - (Time + Dist), R1),
...
find_best_customer(Taxi_info, [Customer], Customer_id, Value, Path, Arival_time):-
	get_taxi_info(Taxi_info, Taxi_position, Current_time),
	get_customer_info(Customer, Customer_id, Customer_position, Online_time, Offline_time, _),
...
find_best_customer(Taxi_info, [First_customer|Customer_list], Customer_id, Value, Path, Arival_time):-
	...
	get_taxi_info(Taxi_info, Taxi_position, Current_time),
	get_customer_info(First_customer, Customer_id3, Customer_position, Online_time, Offline_time, _),
